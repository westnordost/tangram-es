...
Texture::Texture(unsigned int _width, unsigned int _height, Options _options, float _density)
    : m_options(_options) {
...
Texture::Texture(const std::vector<char>& _data, Options options, float density)
    : Texture(0u, 0u, options, density) {
...
void Texture::resize(const unsigned int _width, const unsigned int _height) {
    ...
    if (!(Hardware::supportsTextureNPOT) &&
        ...
        (m_options.generateMipmaps || (m_options.wrapS == Wrap::REPEAT || m_options.wrapT == Wrap::REPEAT))) {
        ...
        m_options.minFilter = MinFilter::LINEAR;
        m_options.magFilter = MagFilter::LINEAR;
